...
def data_preprocess():
    ...
    rating_matrix = pd.DataFrame(
        rating_matrix, index=user_id, columns=movie_id)

    print("Converting data...")
    count = 0
    user_num = len(user_id)
    for uid in user_id:
        user_rating = rating_data[rating_data['userId'] == uid].drop(
    ...
    rating_matrix.to_csv('data/user-rating.csv')
    ...
    movies_data.to_csv('data/movies_feature.csv')
