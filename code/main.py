...
if __name__ == '__main__':
    ...
    movies_feature = pd.read_csv(
        'data/movies_feature.csv', index_col=0)
    user_rating = pd.read_csv(
        'data/user-rating.csv', index_col=0)
    ...
    for idx, user in train.iterrows():
        ...
        for item in unrated_index:
            rate_h = Content_based_recommendation.CB_recommend_estimate(
                user, movies_feature, int(item))
    ...
    train.to_csv('data/pred_ratings_CB.csv')
    ...
    #     rates_lst = Collaborative_Filtering_recommendation.CF_recommend_estimate(
    #         train, idx, unrated_index_, 50)
    ...
    # train.to_csv('data/pred_ratings_CF.csv')

    # 计算基于内容的推荐和协同过滤的MSE和RMSE
    # pred_CB = pd.read_csv('data/pred_ratings_CB.csv', index_col=0)
    # pred_CF = pd.read_csv('data/pred_ratings_CF.csv', index_col=0)
